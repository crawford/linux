...
#ifdef CONFIG_CORESIGHT_QCOM_STM_DEFAULT_ENABLE
			       ...
			       struct device_attribute *attr, char *buf)
{
	...
	len = scnprintf(buf, PAGE_SIZE, "%*pb", OST_ENTITY_MAX,
			&drvdata->entities);
...
static int stm_probe(struct platform_device *pdev)
{
	...
	drvdata->write_64bit = device_property_read_bool(dev,
							 "qcom,write-64bit");
	drvdata->data_barrier = device_property_read_bool(dev,
							  "qcom,data-barrier");
...
static const struct acpi_device_id stm_acpi_match[] = {
	{ "ARMHFFF1"  },
	{ },
};
MODULE_DEVICE_TABLE(acpi, stm_acpi_match);
#endif
...
static struct platform_driver stm_driver = {
	...
	.driver         = {
		...
		.acpi_match_table = ACPI_PTR(stm_acpi_match),
	},
};

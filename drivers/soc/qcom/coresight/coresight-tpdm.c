...
/* Utility functions for read, modify write */
static inline void tpdm_clear_bit(struct tpdm_drvdata *drvdata,
			int pos, int reg)
{
	uint32_t config;

	config = tpdm_readl(drvdata, reg);
	config &= ~BIT(pos);
	tpdm_writel(drvdata, config, reg);
}

static inline void tpdm_set_bit(struct tpdm_drvdata *drvdata,
			int pos, int reg)
{
	uint32_t config;
...
static void __tpdm_enable_bc(struct tpdm_drvdata *drvdata)
{
	...
	/* Set the enable bit */
	tpdm_set_bit(drvdata, 0, TPDM_BC_CR);
}
...
static void __tpdm_enable_tc(struct tpdm_drvdata *drvdata)
{
	...
	/* Set the enable bit */
	tpdm_set_bit(drvdata, 0, TPDM_TC_CR);
}
...
static void __tpdm_enable_dsb(struct tpdm_drvdata *drvdata)
{
	...
	/* Set the enable bit */
	tpdm_set_bit(drvdata, 0, TPDM_DSB_CR);
}
...
static void __tpdm_enable_cmb(struct tpdm_drvdata *drvdata)
{
	...
	if (drvdata->cmb->mode == TPDM_CMB_MODE_CONTINUOUS)
		...
		val = val | BIT(1);
	/* Set the enable bit */
	tpdm_set_bit(drvdata, 0, TPDM_CMB_CR);
}
					 ...
					 char *buf)
{
	...
	size = scnprintf(buf, PAGE_SIZE, "%*pb", TPDM_DATASETS,
			drvdata->enable_ds);
				 ...
				 char *buf)
{
	...
	mutex_lock(&drvdata->lock);
	for (i = 0; i < TPDM_GPR_REGS_MAX && size < PAGE_SIZE; i++) {
		if (!test_bit(i, drvdata->gpr->gpr_dirty))
			continue;
		size += scnprintf(buf + size, PAGE_SIZE - size,
					...
					size_t size)
{
	...
	mutex_lock(&drvdata->lock);
	for (i = start; i <= end; i++) {
		GET_REG_BIT(reg, bit, i, 2);
		bit = bit * 2;

		drvdata->dsb->edge_ctrl[reg] |= edge_ctrl << bit;
	}
	mutex_unlock(&drvdata->lock);
	return size;
}
				   ...
				   size_t size)
{
	...
	mutex_lock(&drvdata->lock);
	if (!strcmp(buf, "continuous")) {
		drvdata->cmb->mode = TPDM_CMB_MODE_CONTINUOUS;
	} else if (!strcmp(buf, "trace_on_change")) {
...
{
	/*  Enable retrieval mode, timestamp requests based on datasets */
	if (test_bit(TPDM_DS_BC, drvdata->datasets))
		drvdata->bc->retrieval_mode = TPDM_MODE_ATB;
...
static const struct acpi_device_id tpdm_acpi_match[] = {
	{ "QCOM80F1"  },
	{ },
};
MODULE_DEVICE_TABLE(acpi, tpdm_acpi_match);
#endif
...
static struct platform_driver tpdm_driver = {
	...
	.driver         = {
		...
		.acpi_match_table = ACPI_PTR(tpdm_acpi_match),

	},
};

module_platform_driver(tpdm_driver);

MODULE_LICENSE("GPL v2");
MODULE_DESCRIPTION("Coresight Trace, Profiling & Diagnostic Monitor driver");

...
#ifdef CONFIG_CORESIGHT_QCOM_CTI_SAVE_DISABLE
...
static int cti_probe(struct platform_device *pdev)
{
	...
	drvdata->cpu = -1;
	if (IS_ENABLED(CONFIG_OF) && dev->of_node) {
		cpu_phandle = of_get_property(pdev->dev.of_node,
					      "coresight-cti-cpu",
					      NULL);
		if (cpu_phandle) {
	...
	if (pdev->dev.of_node && !cti_save_disable)
		drvdata->cti_save = device_property_read_bool(&pdev->dev,
							      "qcom,cti-save");
	if (drvdata->cti_save) {
		...
		drvdata->cti_hwclk = device_property_read_bool(&pdev->dev,
							       "qcom,cti-hwclk");
	}
...
static const struct acpi_device_id cti_acpi_match[] = {
	{ "ARMHFFF5"  },
	{ },
};
MODULE_DEVICE_TABLE(acpi, cti_acpi_match);
#endif
...
static struct platform_driver cti_driver = {
	...
	.driver         = {
		...
		.acpi_match_table = ACPI_PTR(cti_acpi_match),
	},
};

...
static const struct reg_entry sgmii_laned[] = {
	/* CDR Settings */
	...
	{0x060, 0x02},         /* SGMII_LN_TX_BAND_MODE */
	{0x19C, 0x03},         /* SGMII_LN_RX_BAND */
	{0x064, 0x1A},         /* SGMII_LN_LANE_MODE */
	{0x1B8, 0x60},         /* SGMII_LN_RX_RCVR_PATH1_MODE0 */
	{0x1F0, 0x03},         /* SGMII_LN_RSM_CONFIG */
};

static const struct reg_entry sgmii_phy[] = {
	{0x080, 0x01},	/* SGMII_PHY_LN_POW_DWN_CTRL0 */
	{0x058, 0x0F},	/* SGMII_PHY_LN_CDR_CTRL0 */
	{0x00C, 0x00},	/* SGMII_PHY_LN_TX_PWR_CTRL */
	{0x018, 0x40},	/* SGMII_PHY_LN_LANE_CTRL1: */
};
...
static void sgmii_v2_write_regs(struct emac_hw *hw, u8 base,
				const struct reg_entry *r, unsigned int size)
{
	unsigned int i;

	for (i = 0; i < size; i++, r++)
		emac_reg_w32(hw, base, r->offset, r->value);
}
...
int emac_hw_init_sgmii_v1(struct emac_hw *hw)
{
	int i;
...
static int emac_hw_init_sgmii_v2(struct emac_hw *hw)
{
	u32 lnstatus, i;
	...
	/* PCS lane-x init */
	sgmii_v2_write_regs(hw, EMAC_SGMII_PHY, sgmii_phy, ARRAY_SIZE(sgmii_phy));

	/* SGMII lane-x init */
	sgmii_v2_write_regs(hw, EMAC_QSERDES, sgmii_laned, ARRAY_SIZE(sgmii_laned));

	/* Power up PCS and start reset lane state machine */
	emac_reg_w32(hw, EMAC_SGMII_PHY, EMAC_SGMII_PHY_RESET_CTRL, 0);
	emac_reg_w32(hw, EMAC_QSERDES, SGMII_LN_RSM_START, 1);

	/* Wait for c_ready assertion */
	for (i = 0; i < SERDES_START_WAIT_TIMES; i++) {
		lnstatus = emac_reg_r32(hw, EMAC_SGMII_PHY, SGMII_PHY_LN_LANE_STATUS);
		if (lnstatus & 0x02)
			break;
		usleep_range(100, 200);
	}
	...
	if (i == SERDES_START_WAIT_TIMES) {
		emac_err(hw->adpt, "SGMII failed to start\n");
		return -EIO;
	}

	/* Disable digital and SERDES loopback */
	emac_reg_w32(hw, EMAC_SGMII_PHY, SGMII_PHY_LN_BIST_GEN0, 0);
	emac_reg_w32(hw, EMAC_SGMII_PHY, SGMII_PHY_LN_BIST_GEN2, 0);
	emac_reg_w32(hw, EMAC_SGMII_PHY, SGMII_PHY_LN_CDR_CTRL1, 0);
...
int emac_hw_reset_sgmii(struct emac_hw *hw)
{
	...
	if (hw->phy_version == SGMII_PHY_VERSION_2)
		return emac_hw_init_sgmii_v2(hw);
	else if (hw->phy_version == SGMII_PHY_VERSION_1)
		return emac_hw_init_sgmii_v1(hw);
...
int emac_hw_init_phy(struct emac_hw *hw)
{
	...
	if (hw->adpt->phy_mode == PHY_INTERFACE_MODE_SGMII) {
		if (hw->phy_version == SGMII_PHY_VERSION_2)
			retval = emac_hw_init_sgmii_v2(hw);
		else if (hw->phy_version == SGMII_PHY_VERSION_1)
			retval = emac_hw_init_sgmii_v1(hw);

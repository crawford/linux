...
static int emac_reset(struct emac_adapter *adpt);
...
static struct emac_clk_info emac_clk[EMAC_NUM_CLK] = {
	{ NULL, "axi_clk", EMAC_AXI_CLK, 0, NULL },
	{ NULL, "cfg_ahb_clk", EMAC_CFG_AHB_CLK, 0, NULL },
	{ NULL, "125m_clk", EMAC_125M_CLK, 0, NULL },
	{ NULL, "25m_clk", EMAC_SYS_25M_CLK, 0, NULL },
	{ NULL, "tx_clk", EMAC_TX_CLK, 0, NULL },
	{ NULL, "rx_clk", EMAC_RX_CLK, 0, NULL },
	{ NULL, "sys_clk", EMAC_SYS_CLK, 0, NULL },
};
...
static int emac_acpi_clk_set_rate(struct emac_clk_info *clk_info, u64 rate)
{
	struct device *dev = clk_info->adpt->dev;
	union acpi_object params[4], args;
	union acpi_object *obj;
	int ret = 0;

	params[0].type = ACPI_TYPE_INTEGER;
	params[0].integer.value = clk_info->index;
	...
	obj = acpi_evaluate_dsm(ACPI_HANDLE(dev), duuid, 1, 2, &args);
	if (!obj)
		return -EINVAL;
...
static phy_interface_t emac_phy_interface_type(const char *phy_mode)
{
	...
	else if (!strcasecmp(phy_mode, "sgmii"))
		return PHY_INTERFACE_MODE_SGMII;
	else if (!strcasecmp(phy_mode, "rgmii"))
		return PHY_INTERFACE_MODE_RGMII;
	else if (!strcasecmp(phy_mode, "xgmii"))
		return PHY_INTERFACE_MODE_XGMII;
	else if (!strcasecmp(phy_mode, "rgmii-id"))
		return PHY_INTERFACE_MODE_RGMII_ID;
	else if (!strcasecmp(phy_mode, "rgmii-txid"))
		return PHY_INTERFACE_MODE_RGMII_TXID;
	else if (!strcasecmp(phy_mode, "rgmii-rxid"))
		return PHY_INTERFACE_MODE_RGMII_RXID;
	else if (!strcasecmp(phy_mode, "tbi"))
		return PHY_INTERFACE_MODE_TBI;
	else if (!strcasecmp(phy_mode, "rmii"))
		return PHY_INTERFACE_MODE_RMII;
	else if (!strcasecmp(phy_mode, "rtbi"))
				    ...
				    struct emac_adapter *adpt)
{
	...
	ret = device_property_read_u32(dev, "phy-channel", &adpt->hw.phy_addr);
	if (ret < 0)
		adpt->hw.phy_addr = 0;

	ret = device_property_read_u8_array(dev, "mac-address", maddr,
					    ETH_ALEN);
	if (ret < 0) {
	...
	ret = device_property_read_u32(dev, "phy-version",
				       &adpt->hw.phy_version);
	if (ret < 0)
		adpt->hw.phy_version = 0;

	adpt->no_ephy = device_property_read_bool(dev, "no-ephy");
	adpt->tstamp_en = device_property_read_bool(dev, "tstamp-eble");
	ether_addr_copy(adpt->hw.mac_perm_addr, maddr);
	return 0;
}

static int emac_acpi_reset_mac(struct device *dev)
{
	union acpi_object *obj;
	int err = 0;

	/* Execute DSM function to reset the MAC through GCC */
	obj = acpi_evaluate_dsm(ACPI_HANDLE(dev), duuid, 1, 1, NULL);
	if (!obj) {
		dev_err(dev, "failed to evaluate _DSM method\n");
		return -ENXIO;
	}
	if ((obj->type != ACPI_TYPE_INTEGER) || (obj->integer.value)) {
		dev_err(dev, "failed to excute _DSM reset, error %d %p\n",
			obj->type, &obj->integer.value);
		err = -EINVAL;
	}
	ACPI_FREE(obj);
	return err;
				   ...
				   struct emac_adapter *adpt)
{
	...
	for (i = 0; i < NUM_EMAC_REG_BASES; i++) {
		...
		/* Firmware initializes the SGMIIv2 common part for us, so
		 * we no longer need to do that.  Skip the 4th entry in
		 * the ACPI tables.  The upstream version of this driver will
		 * parse ACPI differently, so this code is temporary.
		 */
		if ((adpt->hw.phy_version == SGMII_PHY_VERSION_2) && (i == 3))
			continue;
		 ...
		 * and one per-lane analog part. The PHY regmap is compatible
		 * to SGMII-v1 controller and will be used in PHY common code.
		 * We re-purpose the EMAC_QSERDES register to hold the per-lane
		 * digital register block.
		 */
		if ((i == EMAC_SGMII_PHY) &&
		    (adpt->hw.phy_version == SGMII_PHY_VERSION_2)) {
			adpt->hw.reg_addr[EMAC_QSERDES] = regmap;
...
static int emac_clk_prepare_enable(struct emac_clk_info *clk_info)
{
	...
	if (ACPI_HANDLE(clk_info->adpt->dev))
		return 0;
...
static int emac_clk_set_rate(struct emac_clk_info *clk_info, unsigned long rate)
{
	...
	if (ACPI_HANDLE(clk_info->adpt->dev))
		return emac_acpi_clk_set_rate(clk_info, rate);
			      ...
			      struct emac_adapter *adpt)
{
	...
	static const char *res_name[NUM_EMAC_REG_BASES] =
		{"emac", "emac_csr", "emac_1588", "emac_qserdes", "emac_sgmii_phy"};
	const void *maddr;
	...
	/**
	 * DTS based driver probe doesn't support SGMII-v2 controller,
	 * so set the SGMII phy-version to v1.
	 */
	if (adpt->phy_mode == PHY_INTERFACE_MODE_SGMII)
		adpt->hw.phy_version = SGMII_PHY_VERSION_1;
...
/* Reset the MAC controller and setup all clocks for normal operation */
static int emac_reset(struct emac_adapter *adpt)
{
	struct emac_hw *hw = &adpt->hw;
	bool ptp_rst = (CHK_HW_FLAG(PTP_EN)) ? true : false;
	int retval;
	...
	if (ACPI_HANDLE(adpt->dev)) {
		retval = emac_acpi_reset_mac(adpt->dev);
		if (retval) {
			dev_err(adpt->dev, "reset failed %d\n", retval);
			return retval;
		}
	}
	...
	if (retval) {
		dev_err(adpt->dev, "failed to initialize the clocks\n");
		return retval;
	}

	retval = emac_hw_init_phy(&adpt->hw);
	if (retval) {
		dev_err(adpt->dev, "failed to initialize the phy\n");
		return retval;
	}

	retval = emac_enable_clks(adpt);
	if (retval) {
		dev_err(adpt->dev, "failed to enable the clocks\n");
		return retval;
	}
...
static int emac_probe(struct platform_device *pdev)
{
	...
	/* init MAC controller & setup default clocks */
	retval = emac_reset(adpt);
	if (retval)
		goto err_reset;

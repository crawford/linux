...
static int msi_override = -1;
module_param(msi_override, int, 0644);
MODULE_PARM_DESC(msi_override, "MSI capability override (0=force off, 1=force on)");
...
static int hidma_probe(struct platform_device *pdev)
{
	...
	/* allow user to disable MSI feature */
	if (msi_override >= 0)
		msi = !!msi_override;
	else
		msi = device_property_read_bool(&pdev->dev, "msi-support");
